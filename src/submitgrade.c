...
if (argc==1) error(argv[0]);
      if (argv[1]==NULL) error(argv[0]); else 
	      N = atoi(argv[1]);
      if (argv[5]==NULL) error(argv[0]); else 
	      P = atoi(argv[5])-1;
...

  {
    ...
    long int vote;
    mpz_init (z);
    if (argv[4]==NULL) error(argv[0]);
    else read_element_from_file(g,argv[4]);
    ...
    else read_element_from_file(secret_key,argv[2]);
   for (i=0;i<N;i++){
    if (argv[6+i]==NULL) error(argv[0]);
    else read_element_from_file(public_key[i],argv[6+i]);
   }
	...
	while (1)
	  {
	    ...
	    if (str[1] == '\0')
	      break;
	    printf ("Invaid choice. Press %s1 for computing an Average Grade,%s 2 for Dead or Alive elections,%s 3 for Victory by unanimity,%s q to quit%s ", KCYN, KMAG, KYEL, KGRN, KWHT);
	    flush ();
	  }
	switch (str[0])
	  {
