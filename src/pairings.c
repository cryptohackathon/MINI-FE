...
mpz_t y2;
mpz_init(y2);
 if (mpz_cmp_si(y[0].z,0)==0) mpz_from_BIG_256_56(y2, (int64_t *) CURVE_Order_BN254);
 else mpz_set(y2,y[0].z);
...
e[0].type=a[0].type;
mpz_set(e[0].p,a[0].p);
    if (e[0].type==0) ECP_BN254_copy(&(e[0].g1),&(a[0].g1));
    else if (e[0].type==1) ECP2_BN254_copy(&(e[0].g2),&(a[0].g2));
    else if (e[0].type==2)FP12_BN254_copy(&(e[0].gT),&(a[0].gT));
    else if (e[0].type==4) mpz_set(e[0].z,a[0].z);
}

void element_mul(element_t c,element_t a, element_t b) { /* c=ab - we use multiplicative notation for the group */
...
mpz_mod(c[0].z,c[0].z,c[0].p);
 if (mpz_cmp_si(c[0].z,0)==0) mpz_from_BIG_256_56(c[0].z, (int64_t *) CURVE_Order_BN254);

}
}

void element_add(element_t c,element_t a, element_t b) { /* c=a+b  */
if (c[0].type!=4) element_mul(c,a,b);
else{
...
void element_sub(element_t c,element_t a, element_t b) { /* c=a-b */
if (c[0].type!=4) element_div(c,a,b);
else{
...
if (mpz_cmp_ui(a[0].z,b)==0) return 1; else return 0;
}
int element_cmp_si(element_t a,signed long int b){
if (mpz_cmp_si(a[0].z,b)==0) return 1; else return 0;
}
int element_cmp(element_t a,element_t b){ /* 0 iff a==b */
if (a[0].type==0) return !ECP_BN254_equals(&(a[0].g1),&(b[0].g1));
else if (a[0].type==1) return !ECP2_BN254_equals(&(a[0].g2),&(b[0].g2));
else if (a[0].type==2) return !FP12_BN254_equals(&(a[0].gT),&(b[0].gT));
else if (a[0].type==4) { if (mpz_cmp(a[0].z,b[0].z)==0) return 0; else return 1;}
else return -1;
}
...
void element_set_mpz(element_t a,mpz_t b){
 mpz_set(a[0].z,b);
 if (mpz_cmp_si(a[0].z,0)==0) mpz_from_BIG_256_56(a[0].z, (int64_t *) CURVE_Order_BN254);
}
void _element_set_mpz(element_t a,mpz_t b){
 mpz_set(a[0].z,b);
}
...
void element_pairing(element_t c,element_t a,element_t b){ /* c=e(a,b) */
	...
	else printf("error in pairing, check the parameters\n");
...
int element_is1(element_t a){ /* return 1 iff element is unity of the group or integer 1 in case of type 4 */
	if (a[0].type==2) return FP12_BN254_isunity(&(a[0].gT));
	else if (a[0].type==0) return ECP_BN254_isinf(&(a[0].g1));
	else if (a[0].type==1) return ECP2_BN254_isinf(&(a[0].g2));
	else if (a[0].type==4) return !mpz_cmp_ui(a[0].z,1);
	else return -1;
}
/*
...
void hash_G1(element_t hash, char *str, unsigned int len) {
...
void hash_G2(element_t hash, char *str, unsigned int len) {
    octet tmp_str = {(int) len, 0, str};
    char h[MODBYTES_256_56];
    octet tmp_hash = {0, sizeof(h), h};
    mhashit(SHA256, -1, &tmp_str, &tmp_hash);
...
void hash_Zr(element_t hash, char *str, unsigned int len) { /* this function seems not work */
BIG_256_56 s;
    octet tmp_str = {(int) len, 0, str};
...
void element_from_hash(element_t hash, char *str, unsigned int length){ /* hash not implemented securely yet  */
	char *str2;
...
else if (hash[0].type==0) hash_G1(hash,str2,length);
else if (hash[0].type==1) hash_G2(hash,str2,length);

free(str2);
}

int element_snprintf(char *str, unsigned int length, char *format, element_t g,element_t h,element_t u,element_t v){ /* for the moment we support only what needed by the demo to work, that is we ignore format  */
	char *a1,*a2,*a3,*a4;
    char S[2048];
...
pairing_t p;
	   ...
	   else if (g[0].type==1) element_init_G2(tmp,p);
